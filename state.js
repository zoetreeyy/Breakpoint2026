...
export function autoScheduleMatches(state) {
  ...
  for (const court of idleCourts) {
    ...
    const eligibleMatch = state.matches.find(match => {
      ...
      // BOTH players/teams must be checked in for THIS specific event
      const ev = match.event;
      const isPlayerCheckedIn = (p) => p && (p.checkInStatus ? p.checkInStatus[ev] : p.checkedIn);
      
      if (!isPlayerCheckedIn(p1) || !isPlayerCheckedIn(p2)) return false;
      if (p1Double && !isPlayerCheckedIn(p1Double)) return false;
      if (p2Double && !isPlayerCheckedIn(p2Double)) return false;
...
export function loadMockDataIntoState(state) {
  ...
  state.players = mockNames.map((name, i) => {
    ...
    const isCheckedIn = i < 14;
    const checkInStatus = {};
    playerEvents.forEach(ev => { checkInStatus[ev] = isCheckedIn; });
    ...
    return {
      ...
      checkedIn: isCheckedIn, // keep for legacy compatibility
